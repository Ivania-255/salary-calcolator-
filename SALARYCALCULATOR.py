...
class App:
	...
	def block_of_code(self):
		...
		self.tab1 = Frame(self.tabs_control, bg=self.set.thems['dark']["tab_color"])
	...
	def widgets(self):
		...
		self.check_theem = Checkbutton(self.tab3, text='swich theem', variable=self.enabled, command=self.checkbutton_changed)
	...
	def send_results(self):
		...
		try:
			...
			self.month_res = Label(self.tab2, text=self.res1, font='Arial 14 bold', bg=self.set.thems['light']['output_color'])
			self.year_res = Label(self.tab2, text=self.res2, font='Arial 14 bold', bg=self.set.thems['light']['output_color'])
			self.week_res = Label(self.tab2, text=self.res3, font='Arial 14 bold', bg=self.set.thems['light']['output_color'])
			self.day_res = Label(self.tab2, text=self.res4, font='Arial 14 bold', bg=self.set.thems['light']["output_color"])
			...
			if not self.salary_input.get() == "":
				if self.label:
					...
					self.month_res = Label(self.tab2, text=self.res2, font='Arial 14 bold', bg=self.set.thems['light']["output_color"])
					self.year_res = Label(self.tab2, text=self.res1, font='Arial 14 bold', bg=self.set.thems['light']["output_color"])
					self.week_res = Label(self.tab2, text=self.res3, font='Arial 14 bold', bg=self.set.thems['light']["output_color"])
					self.day_res = Label(self.tab2, text=self.res4, font='Arial 14 bold', bg=self.set.thems['light']["output_color"])
